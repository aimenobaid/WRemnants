...
def make_parser():
    ...
    parser.add_argument("-i","--inputFile",equired=True,type=str, help="Input HDF5 histogram file (output of a histmaker).")
    ...
    parser.add_argument("--fakeSmoothingPolynomial",type=str,default="chebyshev",choices=Regressor.polynomials,help="Polynomial type for the spectrum smoothing.")
    parser.add_argument("--lumiScale",type=float,default=1.0,help="Rescale equivalent luminosity by this value (must match the value used in carrot).")
    parser.add_argument("--excludeProcGroups",type=str,nargs="*",default=["QCD"],help="Process groups to exclude when building Datagroups.")
    parser.add_argument("--filterProcGroups",type=str,nargs="*",default=None,help="If set, keep only these process groups when building Datagroups.")
    return parser
...
def dump_smoothing_params(
    ...
    # Build combined fake histogram: data - prompt MC.
    # This is used only to derive initial polynomial coefficients.
    datagroups.loadHistsForDatagroups(inputBaseName,syst="",procsToRead=[g],label="_dump_tmp",applySelection=False,)
    h_fakes = datagroups.groups[g].hists["_dump_tmp"]

    # Single call with signal_region=False: returns 5 regions [A=0, B=1, C=2]
    # The dropped 4th flat element (D = signal mt + pass iso) is the predicted region.
    ...
    # Shape: (*outer_dims, 3, order+1) — indices [Ax=0, Bx=1, A=2, B=3, C=4]
    params_3d = fakeselector._params_before_reduce.copy()

    reg = fakeselector.spectrum_regressor
    order = reg.order

    # Flatten outer dims → (n_outer_flat, n_abcd, order+1)
    outer_shape = params_3d.shape[:-2]
    n_outer_flat = int(np.prod(outer_shape))
    params_3d_flat = params_3d.reshape(n_outer_flat, 3, order + 1)

    # Assemble the 3 model regions in model order [A, B, C]
    params_model = np.stack(
        [
            params_3d_flat[:, 1, :],  # A_model ← physical B
            params_3d_flat[:, 0, :],  # B_model ← physical A
            params_3d_flat[:, 2, :],  # C_model ← physical C
        ],
        axis=1,
    )  # (n_outer, 3, order+1)
